...
function getModelYears(arr) {
  const modelyear = []    //create empty array
  for (let i = 0; i < arr.length; i++) {  //loop through array one at a time
    modelyear.push(arr[i].car_year) //will add the car's year to the array we created
  }
  return modelyear;  //return the new array
}
